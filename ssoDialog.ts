...
export class SSODialog extends ComponentDialog {
  private requiredScopes: string[] = [config.downstreamApiScopes]; // hard code the scopes for demo purpose only
  ...
  async executeOperationWithSSO(stepContext: any) {
    ...
    const SSOCommand = SSOCommandMap.get("chat");
    if (!SSOCommand) {
      throw new Error("Can not get sso operation. Please try again.");
    }
    await SSOCommand.
    
    operationWithSSOToken(stepContext.context, tokenResponse.token);
